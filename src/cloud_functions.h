...
namespace cloud_functions
{
  ...
  void CloudDownSample(typename pcl::PointCloud<PointT>::Ptr input_cloud, typename pcl::PointCloud<PointT>::Ptr output_cloud, float LS = 0.01f)
  {
    float LeafeSize = LS;
  ...
  template<typename T>
  inline T minimum( T a, T b ) { return a>b ? b : a ; }
  template<typename T>
  inline T maximum( T a, T b, T c ) { return maximum( maximum(a,b), c ) ; }
  template<typename T>
  inline T minimum( T a, T b, T c ) { return minimum( minimum(a,b), c ) ; }
  template<typename T>
  inline T median( T a, T b, T c ) { return maximum(minimum(a,b), minimum(maximum(a,b),c));}
  template<typename T>
