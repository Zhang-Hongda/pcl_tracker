...
void 
fit_sphere(hsvCloudPtr in, hsvCloudPtr cloud_sphere, pcl::ModelCoefficients::Ptr coefficients_sphere)
{
  ...
  pcl::ExtractIndices<hsvRefPointType> extract;  // Write the sphere inliers to disk
...
bool 
ball_extraction (hsvCloudPtr cloud, std::vector<hsvCloudPtr>& cloud_clusters, std::vector<hsvCloudPtr>& sphere_cloud_set, std::vector<pcl::ModelCoefficientsPtr>& coefficients_sphere_set)
{
  if(cloud->size() == 0)
    return(false);
